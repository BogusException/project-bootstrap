...
## 2. Entry point and screen handling
...
|---|-------|-------------|-----------------|
| 1 | Preflight | Checks Python 3.12+, git, jq; refuses if project has existing work; asks y/N to proceed | Exits if >1 commit, or 1 commit that is not the scaffold message |
| 2 | Git | `git init -b main`; bare repo and remote are optional (user configures later -- see adding-remotes.md) | Skips if `.git` already exists |
...
| 8 | Commit | `git add -A`, commits "Initial project scaffold" | Skips if HEAD already exists |
| 9 | Verify | Confirms all hooks are executable (fixes silently), prints next steps | -- |

## 4. Hook inventory

### 4.1 Copied from dotclaude marketplace

Source: `~/.claude/plugins/marketplaces/dotclaude/hooks/`
...
### 4.2 Bundled in `templates/hooks/`
