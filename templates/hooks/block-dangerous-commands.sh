...
# Or create .claude/protected-branches in the project root (one branch per line,
# empty file = branch protection disabled for this repo).
...
CONFIG_FILE="${PROJ_ROOT}/.claude/protected-branches"

if [ -f "$CONFIG_FILE" ]; then
  # Config file present: read comma or newline-separated list.
  # Empty file = branch protection disabled for this repo.
  PROTECTED_BRANCHES=$(tr '\n' ',' < "$CONFIG_FILE" | sed 's/,$//')
elif [ -n "${CLAUDE_PROTECTED_BRANCHES+x}" ]; then
  PROTECTED_BRANCHES="$CLAUDE_PROTECTED_BRANCHES"
else
  PROTECTED_BRANCHES="$DEFAULT_BRANCHES"
fi
